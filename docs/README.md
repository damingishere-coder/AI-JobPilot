...
这里汇总 AI JobPilot 的使用、开发、架构、安全、版本发布和演示资料。当前应用为 **1.5.0**，配套 Chrome Bridge 为 **1.8.0**；Windows 日常使用的页面和 API 统一在 **http://127.0.0.1:6866**。

第一次使用：按 [Windows 安装指南](../WINDOWS_SETUP.md) 准备环境，再跟随 [首次使用与任务流程](../TASK_FLOW.md) 完成一轮岗位分析。需要下载文件时，查看 [Release 产物选择与校验](releases.md)。

详细开发说明与历史设计分开维护。`doc/` 和带日期的旧计划用于追溯，不代表当前功能、端口或发布状态；遇到冲突，以当前版本指南和实际配置为准。
...
| [1.5 发布说明](releases/v1.5.0.md) | 当前版本来源、配套扩展、升级与回滚 |
| [新版界面截图](images/screenshots/README.md) | 真实前端构建、虚构数据与图片来源 |
