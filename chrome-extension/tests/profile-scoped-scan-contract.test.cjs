...
test("extension release and both content scripts use the profile-scoped contract", () => {
  ...
  assert.match(background, /BACKGROUND_VERSION = "2026-09-08-reliable-submit"/);
  ...
  assert.match(zhilian, /EXTENSION_VERSION = "2026-09-08-reliable-submit"/);
  assert.match(background, /REQUIRED_ZHILIAN_CONTENT_VERSION = "2026-09-08-reliable-submit"/);
});
