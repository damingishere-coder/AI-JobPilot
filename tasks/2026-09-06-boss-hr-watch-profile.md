...
## 2026-09-06 全部会话读取与半小时值守

用户授权逐个打开全部列表、读取聊天并生成待确认回复，新增每半小时检查全部会话。扩展 1.6.4 提供两种值守范围：每 30 分钟全部会话（默认选项，含已读），每 1 分钟仅未读。另有“立即读取全部并生成草稿”。启动后执行第一轮，后续从上一轮完成时间再等待指定间隔，不重入、不与发送并行。全列表读取只操作现有聊天标签，不刷新页面；每次打开后等待 2 秒，逐条提交并确认 Outbox，显示已处理数量。生成或保存结果未知时暂停，不自动重试。全列表最多 1000 会话、25 分钟；未确认读到列表底部时明确暂停，不宣称全部完成。

后端接受 1 / 30 分钟间隔并在状态中返回。已读历史经新扩展重新采集后，正确的消息类型形成新来源指纹，旧错误草稿自然过期；同一来源仍去重，不周期性重复调用 AI。使用当前采集的有序上下文生成，避免混入之前图片误判的历史副本。最后一条是本人发送的会话仅保存记录，关闭旧待回复任务，不再次回复更早 HR 消息；已领取或结果未知的发送记录不会被改写。

扩展更新监听器可以把新助手注入现有聊天页，停止旧值守，保留手动启动，不需要 BOSS 页面刷新。真实使用仍须先保证运行服务与扩展版本一致。

验证：后端全套 326 项（322 通过、4 跳过），bootJar 通过；使用 target/hr-all-build 独立构建目录，未触发线上 devtools 重启。扩展 109 项、前端 52 项通过，类型检查、涉及文件 lint、扩展清单和脚本语法通过。测试包含半小时间隔、全列表含已读、逐条保存、提交结果未知后停止、过期会话拒绝、已回复会话不再生成。

真实操作状态：只读查看了现有 Chrome/BOSS 页面；Windows Computer Use 在读取 Chrome 窗口状态时无法可靠确认 URL，终止了本轮桌面操作。未执行全部列表点击、未生成新的真实草稿、未发送 HR/QQ 消息。没有绕过工具拦截，未重启服务或更新实际扩展。代码已准备，运行切换和第一轮全列表验收待继续。

回滚：停止值守后 git revert 本次功能提交，恢复前一服务/扩展版本；不覆盖共享数据库，保留历史记录。部署仍采用前述共享数据库在线备份、只重启 RunDock Backend、核验 PID/数据库/健康与值守接口的流程。


## 2026-09-06 全列表读取后的 QQ 同步

用户明确要求全部列表读取后同步 QQ。NapCat 已在线，QQ 通知已启用，目标群连接测试已获得消息 ID 和回读匹配。调整草稿通知条件：启用 QQ 时同步所有新生成的待确认记录，不再只推送 highValue 记录；关闭 QQ 时不发送通知。保持来源去重、本人已回复会话不重复生成、真实 BOSS 发送逐条确认。

新增普通 REPLY / highValue=false 也会调用 QQ 通知的测试；相关值守和 NapCat 测试及 bootJar 通过，使用独立构建目录。Windows Computer Use 再次因不能可靠确认 Chrome URL 而中止桌面操作，未执行聊天点击。服务更新使用已授权的现有 RunDock Backend 入口，不改数据库路径或密钥；切换前在线备份位于共享 target/backups/hr-all-qq-20260906-215648，完整性正常，profile=3、boss_data=895、hr_message=82、hr_reply_proposal=23。

本次最终仍须区分代码与运行验证：同步规则的真实执行，需在新版后端运行后由用户点击一次开始值守。不能把测试消息送达当成全列表同步完成。回滚本次通知规则可 git revert 本提交；服务回退按已有备份流程执行，不覆盖共享数据库。
