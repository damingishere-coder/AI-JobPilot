...
用户指出频繁刷新有风控风险后，已停止 BOSS 页面操作，后续仅离线开发和测试。未在真实页面加载 1.6.2，未重新开始值守，未生成真实草稿、发送 HR 或 QQ 消息。下一次真实验收须与用户约定一次受控扩展更新和聊天页刷新，不连续重试。用户已明确人物档案名称仅为标签，不要求与简历姓名或 BOSS 显示名一致，不增加姓名匹配拦截。

本补丁回滚可 `git revert` 对应 DOM 兼容修复提交，再在约定时间更新扩展；数据库保持原样。生产后端运行版本无需因这个扩展补丁重启。


## 2026-09-06 文本误判与空草稿原因提示

用户自行加载 1.6.2 后已成功开始值守，Chrome 标签绑定、扫描和入库均成功，Outbox 为 0。只读检查本地状态与草稿接口确认 22 条记录均为 NEEDS_USER / NON_TEXT_MESSAGE，未进入 AI 生成；并非 22 份生成成功的回复。公开 BOSS v5535 渲染代码把 MessageAvatar（figure）与文字放在同一个 message-content 中，旧版任意 img 判断将头像当成图片消息。

1.6.3 将当前消息行自身 ChatMessage 组件的 messageType 映射为消息类型，并核对 $el 与 data-mid，节点复用时先清除过期信息。头像、引用图片、行内表情不改变消息类型；未知类型仍需人工查看。正文排除引用与界面状态，保留表情 alt 和换行。面板展示 summary 与 missingFacts，明确区分需人工查看、草稿生成失败、待补充信息和无需回复；空草稿继续禁止发送。

测试：扩展 107 项通过，前端全部 51 项通过（DOM 10 项、面板 1 项），typecheck 和涉及文件 lint 通过；扩展清单/脚本语法及 diff 检查通过。未修改后端、数据库或服务配置，未重启服务，未操作或刷新 BOSS 页面，未发送 HR/QQ 消息。22 条旧记录原样保留，不自动改类型、重放或重新生成。用户可跳过不需要回复的旧记录；新版真正的文字消息到 AI 草稿流程仍待用户安排单次验收。档案名称只是标签，无须对应简历或 BOSS 姓名。

更新步骤：先停止值守，确认停止且 Outbox 为 0，再重新加载 Bridge 1.6.3；只刷新已有聊天页一次，手动开始值守。优先等待下一条正常文字消息，不以重复刷新或反复打开历史会话验收。看到非空草稿仅说明生成成功，实际发送仍需逐条确认。回滚本次提交可使用 git revert，保留历史数据库；更新扩展应在停止值守后进行。
