...
public class Boss {
    ...
    private void postJobByCity(String cityCode) {
        ...
        for (String keyword : config.getKeywords()) {
            ...
            for (int i = 0; i < count && analyzedCount < searchJobLimit; i++) {
                ...
                analysisRequest.setCompanyName(job.getCompanyName());
                analysisRequest.setJobName(job.getJobName());
                analysisRequest.setSalary(job.getSalary());
                analysisRequest.setLocation(tags.isEmpty() ? "" : tags.get(0));
                analysisRequest.setExperience(tags.size() > 1 ? tags.get(1) : "");
                analysisRequest.setDegree(tags.size() > 2 ? tags.get(2) : "");
                analysisRequest.setCompanyInfo(bossJobTitle != null ? bossJobTitle : "");
                analysisRequest.setJobDescription(job.getJobInfo());
    ...
    private void resumeSubmission(String keyword, Job job) {
        resumeSubmission(keyword, job, null);
    }

    private void resumeSubmission(String keyword, Job job, JobAiAnalysisService.AnalysisResult analysis) {
        // 若收到停止指令，直接短路返回
        if (shouldStopCallback != null && Boolean.TRUE.equals(shouldStopCallback.get())) {
        ...
        // 5. AI智能生成打招呼语
        String aiMessage = analysis != null && isValidString(analysis.getGreeting()) ? analysis.getGreeting() : null;
        if (!isValidString(aiMessage) && config.getEnableAI()) {
    ...
    private String extractEncryptIdFromCurrentDetailLink() {
        try {
            Locator moreInfoBtn = page.locator("a.more-job-btn");
            if (moreInfoBtn.count() == 0) return null;
            String href = moreInfoBtn.first().getAttribute("href");
            if (href == null) return null;
            return extractEncryptId("https://www.zhipin.com" + href);
        } catch (Exception e) {
            return null;
        }
    }
