...
class FullPipelineBrowserRegressionTest {
    ...
    @BeforeEach void setup() throws Exception {
        ...
        Path background=copy.resolve("background.js");Files.writeString(background,guard+Files.readString(background));
    ...
    @Test void collectAnalyzeConfirmApplyCallbackAndReloadStaySingleAttempt() throws Exception {
        ...
        JsonNode claimed=tree(extension.evaluate("async args=>{window.localToken=(await(await fetch(args.origin+'/api/local-auth/action-token')).json()).data.token;return await BrowserApplicationRuntime.claim({task:args.task,message:{runId:'fixture-confirm',runtimeSessionId:'fixture-session',correlationId:'fixture-correlation'},platform:'boss',pageTabId:args.owner,ownerAlive:async id=>!!await chrome.tabs.get(id),request:async(path,options)=>{const response=await fetch(args.origin+path,{method:options.method,headers:{'Content-Type':'application/json','X-Local-Action-Token':window.localToken},body:JSON.stringify(options.body)});const text=await response.text();window.claimResponse={status:response.status,text};return {success:response.ok,data:JSON.parse(text)};}});}",Map.of("task",JSON.convertValue(task,Map.class),"origin",origin,"owner",owner)));
        assertThat(claimed.has("task")).withFailMessage(claimed+" HTTP: "+extension.evaluate("window.claimResponse")).isTrue();JsonNode runtimeTask=claimed.path("task");assertThat(runtimeTask.path("runtime").path("claimVersion").asInt()).isEqualTo(1);
